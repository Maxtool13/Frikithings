...
<html>
        ...
        <head>
            ...
            <title>EDICION DE FIGURAS</title>
        </head>
        ...
        <div class="bg-cover clearfix pt-3">
            <h2 class="logo">FrikiThings</h2> 
                ...
                <br>
                    ...
                    <form action="{{ url('/Figuras/'.$datosfiguras->id)}}" method="POST" id="chucha" enctype="">
                        {{csrf_field() }}
                        {{method_field('PATCH')}}
                            
                        <label for="nombre" id="blin">{{'Nombre'}}</label>
                        <input type="text" name="nombre" id="nombre" value="{{$datosfiguras->nombre}}" disabled="true">
                        <br>

                        <label for="serie" id="blin">{{'Serie'}}</label>
                        <input type="text" id="cuak" name="serie" id="serie" value="{{$datosfiguras->serie}}" disabled="true">
                        <br>

                        <label for="marca" id="blin">{{'Marca'}}</label>
                        <input type="text" id="cuak" name="marca" id="marca" value="{{$datosfiguras->marca}}" disabled="true">
                        <br>

                        <label for="peso(gr)" id="blin">{{'Peso(gr)'}}</label>
                        <input type="number" id="cuak" name="peso" id="peso" value="{{$datosfiguras->peso}}" disabled="true">
                        <br>

                        <label for="altura(cm)" id="blin">{{'Altura(cm)'}}</label>
                        <input type="number" id="cuak" name="altura" id="altura" value="{{$datosfiguras->altura}}" disabled="true">
                        <br>

                        <label for="empaque" id="blin">{{'Empaque'}}</label>
                        <input type="text" id="cuak" name="empaque" id="empaque" value="{{$datosfiguras->empaque}}" disabled="true">
                        <br>

                        <label for="precio" id="blin">{{'Precio MXN'}}</label>
                        <input type="number"  id="cuak" name="precio" id="precio" disabled="true">
                        
                        <label for="imagen" id="blin">{{'Imagen'}}</label>
                        <br>
                        <img src="{{asset('storage').'/'.$datosfiguras->imagen}}" alt="" >
                        <br>

                        <center>
                        <a href="{{ url('/Figuras/'.$datosfiguras->id . '/edit' )}}"><button type="button" class="btn btn-success btn-lg">Ir a Editar</button></a>

                        <a href="/regycon/consultas/configuras"><button type="button" class="btn btn-secondary btn-lg">Regresar a la Consulta</button></a>
